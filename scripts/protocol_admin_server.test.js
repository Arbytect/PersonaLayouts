...
async function run() {
  ...
  assert.strictEqual(atlas.version, '2.1.0');
  assert.strictEqual(atlas.lenses.length, 13);
  assert(atlas.sources.length >= 28);
  assert.strictEqual(atlas.reference_registry.length, 17);
  assert.strictEqual(atlas.comparison_sets.length, 4);
  assert(atlas.hybrid_framework && atlas.local_context);
  assert.strictEqual(atlas.lenses.filter(lens => lens.public.philosophy).length, 13);
  assert.strictEqual(
  ...
  assert.strictEqual(atlasImages.length, 14);
